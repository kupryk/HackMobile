...
public extension AppleMobileDeviceManager {
    ...
    internal func lockdownGetValue<T: Codable>(client: lockdownd_client_t?, domain: String? = nil, key: String? = nil) -> T? {
        ...
        guard let data = Helpers.read_plist_to_binary_data(plist: plist),
              let ret = try? PropertyListDecoder().decode(T.self, from: data) else {
            return nil
        }
    ...
    func obtainDeviceBatteryInfo(udid: String, connection: ConnectionMethod = configuration.connectionMethod) -> BatteryRecord? {
    ...
    func obtainDeviceScreenshot(udid: String, connection: ConnectionMethod = configuration.connectionMethod) -> Data? {
        var result: Data?
        ...
        requireDevice(udid: udid, connection: connection) { device in
            ...
            requireLockdownClient(device: device, handshake: true) { client in
                ...
                requireLockdownService(client: client, serviceName: "com.apple.mobile.screenshotr") { service in
                    ...
                    guard screenshotr_client_new(device, service, &screenshotr_client) == SCREENSHOTR_E_SUCCESS, let screenshotr_client = screenshotr_client else {
                        return
                    }
                    
                    defer {
                        screenshotr_client_free(screenshotr_client)
                    }
