...
public extension AppleMobileDeviceManager {
    ...
    func listApplications(device: idevice_t) -> AnyCodableDictionary? {
        ...
        requireInstallProxyService(device: device) { inst_client in
            ...
            guard let apps, plist_get_node_type(apps) == PLIST_ARRAY else {
                return
            }
            ...
            defer {
                if let sb_client = sb_client {
                    sbservices_client_free(sb_client)
                }
            }
            ...
            for idx in 0 ..< appCount {
                ...
                guard let app_entry = plist_array_get_item(apps, idx), plist_get_node_type(app_entry) == PLIST_DICT else {
                    return
                }

                guard let bundleHandler = plist_dict_get_item(app_entry, "CFBundleIdentifier"), plist_get_node_type(bundleHandler) == PLIST_STRING else {
                    return
                }
                ...
                guard let data = Helpers.read_plist_to_binary_data(plist: app_entry), 
                      var appElement = try? PropertyListDecoder().decode([String: AnyCodable].self, from: data) else {
                    return
                }
                ...
                if let sb_client {
                    ...
                    if sbservices_get_icon_pngdata(sb_client, bundleIdentifier, &buf, &len) == SBSERVICES_E_SUCCESS, let buf = buf, len > 0 {
    ...
    func listApplications(udid: String, connection: ConnectionMethod = configuration.connectionMethod) -> AnyCodableDictionary? {
